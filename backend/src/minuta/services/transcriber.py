...
class Transcriber:
    ...
    async def process_audio(self, pcm_data: bytes, source: str) -> TranscriptSegment | None:
        ...
        if is_speech:
            ...
            # Force split every 10 seconds of audio
            total_samples = sum(len(b) for b in self._buffers[source])
            if self._debug_count % 100 == 0:
                logger.info("Buffer: src=%s chunks=%d samples=%d (%.1fs)",
                    source, len(self._buffers[source]), total_samples, total_samples / self._sample_rate)
            if total_samples >= self._sample_rate * 10:
                logger.info("Force-split: %d samples (%.1fs) for %s", total_samples, total_samples / self._sample_rate, source)
    ...
    async def _flush_buffer(self, source: str) -> TranscriptSegment | None:
        ...
        logger.info("Transcribing: %s, %d samples (%.1fs audio)", source, len(audio), len(audio) / self._sample_rate)
        ...
        if segment:
            logger.info("Transcript: [%s] %s", segment.speaker, segment.text[:80])
            if self._on_segment:
                await self._on_segment(segment)
        else:
            logger.info("Transcription returned empty for %s", source)
